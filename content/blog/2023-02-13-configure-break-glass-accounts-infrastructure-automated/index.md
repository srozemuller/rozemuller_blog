...
It is hightly recommend that you maintain a goal of restricting emergency account use to only the times when it is absolutely necessary.

### Create Break Glass user account
The first step is crateing a break glass account. With the code below I use the Graph API. I generate a strong password based on random characters. The ```Get-RandomPassword``` function can be found at my GitHub page.
...
The reason why I give permenant permissions is because of I can't assign permissions in the case of an emergency. If I'm not able to login anymore, I can't elevate permissions ;). 

In the code below, I assign the global admin role to the user I created above. The role definition ID ```"62e90394-69f5-4237-9190-012177145e10"``` is a builtIn role ID that is available in every tenant and has the same ID. (See [BuiltIn Roles](https://learn.microsoft.com/en-us/azure/active-directory/roles/permissions-reference#global-administrator)) 
...
For more information about assign permissions in PIM using Graph API, check the [Graph API documentation](https://learn.microsoft.com/en-us/graph/api/rbacapplication-post-roleassignmentschedulerequests?view=graph-rest-beta&tabs=http)

## Monitor Account Usage
As disqussed above, the break glass account is a high priveledged user account that must handled with care. Because of that, it is recommended to monitor the account usage. This is the reason why I create a monitor rule to check for account usage.  
The idea is creating an alert rule in Azure Monitor that checks the sign-in logs in the Log Analytics. Every log in step is logged even when the login is not succesful. If the user account is logged, the IT admin gots an email. 
...
Asuming there is a Log Analytics workspace already I use the code below configuring diagnostics settings for Azure AD using the Azure Management API. 
...
### Create Action Group
The next step is creating an action group. An action group is the next step in line after an alert is occured. An alert group contains 'endpoints' where alerts are sent too. This can be a webhook, logic app or an e-mail for example. 
...
The reason why I create an action group first is because when creating an alert rule, I can directly provide the correct action group. 
...
### Create monitor rule
The last step is creating the monitor rule itself. Several alert rule types are available. Think about metrics, activity logs and custom logs. Every type has its own API endpoint. For this type (custom logs), we need the ```Scheduled Query Rules``` endpoint. This sound a bit weird maybe, but in fact the name fits quite well. This is because of this rule type is actually a schedule task. A task that runs every n-minutes. 
...
The complete action rule body is stored below. In the end we have rule that searches every 5 minutes for a sign in log where the user principal name equals the break glass account. 

I configured also a dimension based on the user's SPN. This to receive a splitted overview when more break glass accounts are monitored and used. 

If there is result, a severity 0 (highest) alert will be created and I will be notified at my email address. 
...
## Results
I did a simple test by try to login with invalid credentials. That results in an alert in my mailbox.
...
Because of that, an extra blog post coming with content how to get more control on your break glass accounts.  

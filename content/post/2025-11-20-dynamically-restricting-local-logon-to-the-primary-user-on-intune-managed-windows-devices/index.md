...
## Danger zone! 
Changing SIDs in Allow Local Logon policy can be very very riskful. Meaning, when removing wrong SID's users are not able to log in any more. Because of that, make sure you have at least the following in place.

- LAPS policy that sets the LAPS password
- Policy that enables the administrator account, setting name `Accounts Enable Administrator Account Status`
